...
class Style_Encoder(nn.Module):
    def __init__(self, conv_dim=64, style_dim=8, norm='ln', activation='relu'):
        ...
        for i in range(2): # original: 2
            shared_layers += [ConvBlock(curr_dim, curr_dim, 4, 2, 1, norm=norm, activation=activation)] # (16,16,256), (8,8,256), (4,4,256)

        shared_layers += [nn.AdaptiveAvgPool2d(1)]

        shared_layers += [nn.Conv2d(curr_dim, style_dim, 1, 1, 0)]
...
class Generator(nn.Module):
    ...
    def forward(self, x_A, x_B):
        ...
        s_fB = self.s_encoder(up_m_B*x_B)
        s_bA = self.s_encoder(up_im_A*x_A)
...
class Discriminator(nn.Module):
    ...
    def __init__(self, image_size=128, conv_dim=64, c_dim=5, repeat_num=6):
        ...
        layers = []
        layers += [ConvBlock(3, conv_dim, 4, 2, 1, norm='spectral', activation='lrelu')]

        curr_dim = conv_dim
        for i in range(1, repeat_num):
            layers += [ConvBlock(curr_dim, curr_dim*2, 4, 2, 1, norm='spectral', activation='lrelu')]
            curr_dim = curr_dim * 2

        kernel_size = int(image_size / np.power(2, repeat_num))

        self.main = nn.Sequential(*layers)
        self.conv1 = ConvBlock(curr_dim, 1, 3, 1, 1, norm='spectral', activation='none')
...
class ConvBlock(nn.Module):
    def __init__(self, input_dim, output_dim, k, s, p, dilation=False, norm='in',
        ...
        if dilation:
            if norm == 'spectral':
                self.conv = nn.utils.spectral_norm(nn.Conv2d(input_dim, output_dim, k, s, dilation=p, bias=use_bias))
            else:
                self.conv = nn.Conv2d(input_dim, output_dim, k, s, dilation=p, bias=use_bias)
        else:
            if norm == 'spectral':
                self.conv = nn.utils.spectral_norm(nn.Conv2d(input_dim, output_dim, k, s, bias=use_bias))
            else:
