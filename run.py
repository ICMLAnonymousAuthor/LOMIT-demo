...
class Run(object):
    ...
    def update_G(self, x_A, x_B, label_A, label_B, isTrain=True):
        ...
        # get style
        s_fA = G.s_encoder(self.masked_img(x_A, m_A))
        s_bA = G.s_encoder(self.masked_img(x_A, 1-m_A))
        s_fB = G.s_encoder(self.masked_img(x_B, m_B))
        s_bB = G.s_encoder(self.masked_img(x_B, 1-m_B))
        ...
        s_fAB = G.s_encoder(self.masked_img(x_AB, m_AB))
        s_bAB = G.s_encoder(self.masked_img(x_AB, 1-m_AB))
        s_fBA = G.s_encoder(self.masked_img(x_BA, m_BA))
        s_bBA = G.s_encoder(self.masked_img(x_BA, 1-m_BA))
        ...
        s_reg_fA = G.s_encoder(self.masked_img(x_A, m_A.detach()))
        s_reg_fB = G.s_encoder(self.masked_img(x_B, m_B.detach()))
        s_reg_fAB = G.s_encoder(self.masked_img(x_AB, m_AB.detach()))
        s_reg_fBA = G.s_encoder(self.masked_img(x_BA, m_BA.detach()))
        
        style_reg = - (self.l1_criterion(s_reg_fA, s_reg_fB) + self.l1_criterion(s_reg_fAB, s_reg_fBA))
